...
(() => {{
  ...
  function injectDesktopShell() {{
    ...
    style.textContent = `
      ...
      #mers-desktop-shell[data-step="loket"] .lokets {{ display: grid; }}
      #mers-desktop-shell[data-step="loket"] .cards {{ display: none; }}
    ...
    shell.innerHTML = `
      <div class="panel">
        <h1>MeRS Desktop</h1>
        <p>Masukkan ID 8 digit lalu pilih alur kerja.</p>
        <input id="mers-desktop-uid" inputmode="numeric" maxlength="8" autocomplete="off" placeholder="16756586">
        <div class="cards">
          <button class="card" data-action="tap"><strong>Tap In Scanner</strong><span>Pilih loket, lalu kirim ID ke scanner MeRS.</span></button>
          <button class="card" data-action="cek"><strong>Cek Menu Pesanan</strong><span>Tampilkan detail pesanan dari ID yang dimasukkan.</span></button>
        </div>
        <div class="lokets">
    ...
    function runPending() {{
      const pending = JSON.parse(sessionStorage.getItem('mersDesktopPending') || 'null');
      if (!pending) return;
      sessionStorage.removeItem('mersDesktopPending');
      showContent();
      runWhenReady(() => {{
        if (pending.mode === 'tap') {{
    ...
    shell.querySelector('[data-action="cek"]').addEventListener('click', () => {{
      const uid = uidValue();
      if (!uid) return;
      sessionStorage.setItem('mersDesktopPending', JSON.stringify({{ mode: 'cek', uid }}));
    ...
    shell.querySelector('[data-action="tap"]').addEventListener('click', () => {{
      ...
      shell.dataset.step = 'loket';
    }});
    shell.querySelectorAll('[data-loket]').forEach(button => button.addEventListener('click', () => {{
      const uid = uidValue();
      if (!uid) return;
      sessionStorage.setItem('mersDesktopPending', JSON.stringify({{ mode: 'tap', uid, loket: button.dataset.loket }}));
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            WebviewWindowBuilder::new(
                app,
                "main",
                WebviewUrl::External(Url::parse(SCANNER_URL).unwrap()),
            )
